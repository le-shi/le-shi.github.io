...
1. ssh工具
    ...
    sudo apt -y install deepin-menu expect lrzsz zssh
...
1. 终端 - 支持选中复制
    1. terminator: `sudo apt -y install terminator`
...
1. vpn
    ...
    sudo apt -y install openssl libssl-dev net-tools liblzo2-dev libpam0g-dev
...
1. pdf查看器
    1. okular: `sudo apt -y install okular`
...
1. py工具,py环境
    ...
    1. pip3: `sudo apt -y install python3-pip`， pip3 -V
...
1. 版本控制客户端
    1. git: `sudo apt -y install git`
    1. gitkraken:
    ```bash
    wget -Nc https://release.axocdn.com/linux/gitkraken-amd64.deb
    sudo dpkg -i gitkraken-amd64.deb
    ```
    1. svn: `sudo apt -y install subversion`
    1. RapidSVN: `sudo apt -y install rapidsvn`
...
1. 打包工具
    1. tar: Mint系统自带
    1. zip: Mint系统自带
    1. rar: `sudo apt -y install rar`

1. 下载工具
    1. wget: Mint系统自带
    1. uget: `sudo apt -y install uget`

1. 局域网端口侦测工具
    1. zenmap: `sudo apt -y install zenmap`

1. 其他的东西
    1. 下拉式终端 tilda: `sudo apt -y install tilda`
    ...
    ```
    1. 截图工具
        + ~~Shutter: `sudo apt -y install shutter`~~ --不好用
        + flameshot:
    ```bash
...
1. ftp工具
    1. FileZilla: `sudo apt -y install filezilla`

1. 连接windows
    1. rdesktop: *either-or*
        apt(1.8.3-2build1): `sudo apt -y install rdesktop`
        source(1.9.0):
        ```bash
        sudo apt -y install gcc libkrb5-dev libtasn1-6 libtasn1-6-dev nettle-dev  ibgnutls28-dev libpcsclite-dev
...
1. 笔记
    ...
    ```
1. 光盘刻录
    1. Brasero: `sudo apt -y install brasero`
...
1. 系统监视
    1. [conky](https://github.com/brndnmtthws/conky/) | [configure](https://github.com/erikdubois/Aureola) : `sudo apt -y install hddtemp curl lm-sensors conky-all conky`
...
sudo apt upgrade -y
# 3. 安装软件
sudo apt -y install vim git zsh tree jq nmap iotop python-pip shellcheck
# deepin-wine-ubuntu
...
sudo apt -y install subversion rapidsvn
...
# install conky
sudo apt -y install hddtemp curl lm-sensors conky-all conky
